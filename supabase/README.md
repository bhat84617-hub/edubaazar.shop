...
|---|-----------------------------------------------------------|------------|--------------------|
| 1 | `RLS disabled on public.orders / public.users` **OR** `Policy `Allow all` is overly permissive` (`ERROR`) | `legacy/setup-tables.sql` created `CREATE POLICY "Allow all" FOR ALL USING(true)` — anon could DELETE/UPDATE all orders & users (IDOR/data leak). Also RLS was disabled on some installs. | `supabase/fix-critical-bugs.sql` → `ENABLE ROW LEVEL SECURITY`, revokes anon UPDATE/DELETE grants, re-creates restrictive policies: anon `INSERT+SELECT` only, `service_role` `ALL`, `critical-fixes.sql` same. |
