...
-- Strategy: anon can INSERT only (checkout/register via API), NO anon SELECT/UPDATE/DELETE.
--           service_role has ALL (server API routes with SUPABASE_SERVICE_ROLE_KEY).
--           Matches critical-fixes.sql / supabase-setup.sql. Account page reads orders
--           via GET /api/orders (service_role), never via anon key.
-- ===========================================================================
...
-- Public should NOT have direct table UPDATE/DELETE
...
-- anon can INSERT only (guest checkout via store.tsx placeOrder -> supabase insert).
-- NO anon SELECT: order PII + download URLs only via /api/orders & admin/telegram routes.
