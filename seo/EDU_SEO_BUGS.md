...
- **Status:** Fixed — `/api/admin/login` checks `ADMIN_PASSWORD` env + DB hash, issues HMAC httpOnly session cookie (`ADMIN_SESSION_SECRET`); all admin APIs validate `isValidAdminSession`.
...
- **Status:** Fixed — all SQL scripts aligned: anon INSERT-only on `orders`+`users`, no anon SELECT; `legacy/setup-tables.sql` no longer has allow-all.
...
- **Status:** Fixed — remaining high-risk dual-use listings removed from public catalog (restricted IDs + sensitive-slug splice); `npm run check:crawl` fails if any sensitive slug remains public.
...
- **Status:** Fixed — all 80 product image refs resolve (`img-check.js` → missing 0); `npm run check:assets` passes.
...
- **Status:** Fixed — CountUp initial state = value; only animates on intersect after mount.
...
- **Status:** Fixed — static pages use stable `2026-08-01`; products use `lastUpdated`/`createdAt`; blog posts use post date.
...
- **Status:** Fixed — `hasNonIndexableFilter` sets `robots noindex` for `q`/`sort`/`kind`/`free`/page>1; only clean URLs in sitemap; robots disallows `/search`.
...
- **Status:** Fixed — `next.config.ts` permanent redirect apex `edubaazar.shop` → `www.edubaazar.shop`; `SITE_URL` default is www everywhere. (Verify live DNS/CDN after deploy.)
...
- **Status:** Fixed — only `x-default` remains in `alternates.languages`.
...
- **Status:** Fixed — all private routes have `layout.tsx` with `robots noindex`; `not-found` noindex; robots.txt disallows private paths; none in sitemap.
...
- **Status:** Fixed — `public/images/og-cover.png` (1200x630 branded) is primary OG/Twitter image in root metadata; logo kept as secondary.
...
- **Status:** Fixed — Hero, ProductGallery, ProductCard, homepage, cart, checkout, compare, account/login/register logos, and AIChat product cards use `next/image` with dimensions.
...
- **Status:** Fixed — product FAQ JSON-LD and visible FAQ share one `productFaqs` array; homepage has visible FAQ section matching `homeFaqLd`.
...
- **Status:** Fixed — homepage has visible H1 at top; skip-to-content link added; shop filters have `aria-current`; icon buttons have `aria-label` throughout.
...
- **Status:** Fixed (content pass) — all stale 2024 `lastUpdated` values bumped to staggered 2026-06..2026-08 dates; ongoing ownership cadence still recommended.
...
- **Status:** Fixed — legacy HTML/CSS app moved to `legacy/archive/`; only docs + SQL remain at `legacy/` root; Next.js build ignores legacy.
...
- **Status:** Fixed — `legacy/supabase-config.js` no longer exists; app reads env via `src/lib/config.ts` placeholder-safe client; RLS is least-privilege.
