...
const staticUrls: SeoUrlAudit[] = [
  { url: "/", type: "home", status: "pass", title: "EduBazar.shop — Online Courses, Digital Books & Hacking Tools", indexable: true, inSitemap: true, issues: 0 },
  { url: "/shop", type: "shop", status: "pass", title: "Shop — Online Courses, Books & Hacking Tools", indexable: true, inSitemap: true, issues: 0 },
...
export const seoIssues: SeoIssue[] = [
  ...
  { id: "BUG-002", severity: "critical", title: "Orders/users RLS is permissive", url: "/admin", evidence: "The legacy SQL policies use USING (true) and WITH CHECK (true).", recommendation: "Replace with least-privilege authenticated owner/admin policies.", status: "fixed" },
  ...
  { id: "BUG-008", severity: "high", title: "Canonical host needs one enforced origin", url: SEO_SITE_URL, evidence: "Fetched output includes both apex and www host variants.", recommendation: "Choose one host and permanently redirect the other.", status: "fixed" },
  { id: "BUG-009", severity: "high", title: "No automated crawl regression suite", url: "/sitemap.xml", evidence: "check:crawl covers internal links, assets, sensitive exclusion, robots, JSON-LD; check:crawl:live hits production HTTP URLs.", recommendation: "Run npm run check:crawl:live in CI or post-deploy against the live site.", status: "fixed" },
  { id: "BUG-010", severity: "high", title: "Search Console and GA4 are not connected", url: "/admin/seo", evidence: "GA4 G-EMKR761SSQ is live; NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION env injects the Search Console meta tag (set token after creating property).", recommendation: "Create Search Console property, set NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION, redeploy, then verify.", status: "acknowledged" },
  { id: "BUG-011", severity: "high", title: "Review schema has an invented fallback", url: "/product/*", evidence: "aggregateRating/review JSON-LD fully removed; fabricated ProductTabs reviews replaced with honest empty state; check:crawl fails if aggregateRating returns.", recommendation: "When a real review system exists, emit aggregateRating only from verified records matching visible content.", status: "fixed" },
  ...
  { id: "BUG-014", severity: "medium", title: "Social preview images are inconsistent", url: "/shop", evidence: "Several pages omit page-specific Open Graph images and use a small logo fallback.", recommendation: "Create a 1200x630 fallback and page/product-specific images.", status: "fixed" },
  { id: "BUG-015", severity: "medium", title: "Raw img tags bypass image optimization", url: "/product/*", evidence: "Product cards and detail pages use raw img elements.", recommendation: "Use next/image with explicit dimensions and sizes.", status: "fixed" },
  ...
  { id: "BUG-018", severity: "medium", title: "Product freshness dates are stale", url: "/product/*", evidence: "Catalog lastUpdated values are mostly from 2024.", recommendation: "Review content and store real publish/update dates.", status: "fixed" },
  { id: "BUG-019", severity: "low", title: "Legacy HTML app remains in the repository", url: "/legacy", evidence: "Legacy HTML/CSS app files were moved to legacy/archive/.", recommendation: "Keep only docs + SQL at legacy/ root; ignore legacy in deployment.", status: "fixed" },
  { id: "BUG-020", severity: "low", title: "Legacy Supabase anon config committed", url: "/legacy", evidence: "legacy/supabase-config.js no longer exists; env is placeholder-safe.", recommendation: "Never commit service-role keys; keep RLS least-privilege.", status: "fixed" },
];
