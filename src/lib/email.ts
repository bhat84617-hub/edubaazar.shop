      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#f9f9f9;">
          ...
          <div style="background:white;padding:32px;margin-top:16px;">
            <h2 style="color:#181d27;margin-top:0;">Welcome, ${safeName}! 🎉</h2>
...
export async function sendOrderConfirmation(data: {
  ...
  const itemRows = data.items
    .map((i) => `<tr><td style="padding:8px;border-bottom:1px solid #eee;">${escapeHtml(i.name)}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:center;">${i.qty}</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right;">₹${i.price}</td></tr>`)
    .join("");
  const downloadLinks = Object.entries(data.downloadUrls || {})
    .filter(([, url]) => /^https?:\/\//i.test(url))
    .map(([name, url]) => `<p style="margin:10px 0;"><a href="${escapeAttr(url)}" style="display:inline-block;background:#687975;color:white;padding:11px 20px;text-decoration:none;font-weight:600;">Download ${escapeHtml(name)} →</a></p>`)
    .join("");
  ...
  const emailHtml = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#f9f9f9;">
      ...
      <div style="background:white;padding:32px;margin-top:16px;">
        ...
        <p style="color:#444;">Hi ${escapeHtml(data.name)}, your order <strong>${escapeHtml(data.orderId)}</strong> has been received.</p>
        ...
        ${data.utr ? `<p style="color:#666;">UTR: ${escapeHtml(data.utr)}</p>` : ""}
        ...
        <hr style="border:none;border-top:1px solid #eee;margin:24px 0;" />
        <p style="color:#888;font-size:12px;">Questions? WhatsApp us at 9582501582 or email edubazarshop@gmail.com</p>
      </div>
    </div>
  `;
  ...
  try {
    await r.emails.send({
      from: FROM_EMAIL,
      to: ADMIN_EMAIL,
      subject: `🛒 New Order: ${escapeHtml(data.orderId)} — ₹${data.total}`,
      html: `
        <div style="font-family:Arial,sans-serif;padding:24px;">
          ...
          ${data.utr ? `<p><strong>UTR:</strong> ${escapeHtml(data.utr)}</p>` : ""}
...
export async function sendOrderStatusUpdate(data: {
  ...
  const statusText = data.status === "approved"
    ...
    : "We couldn't process your order. Please contact support on WhatsApp at 9582501582.";
  const downloadLinks = Object.entries(data.downloadUrls)
    .filter(([, url]) => /^https?:\/\//i.test(url))
    .map(([name, url]) => `<p style="margin:10px 0;"><a href="${escapeAttr(url)}" style="display:inline-block;background:#687975;color:white;padding:11px 20px;text-decoration:none;font-weight:600;">Download ${escapeHtml(name)} →</a></p>`)
    .join("");
  ...
  try {
    await r.emails.send({
      from: FROM_EMAIL,
      to: data.email,
      subject: `Order ${escapeHtml(data.orderId)} — ${escapeHtml(data.status.toUpperCase())}`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#f9f9f9;">
          <div style="background:#edece9;padding:24px;text-align:center;border-bottom:3px solid #687975;">
            <h1 style="margin:0;color:#181d27;font-size:22px;">EduBazar.shop</h1>
          ...
          <div style="background:white;padding:32px;margin-top:16px;">
            ...
            <p style="color:#444;">Hi ${escapeHtml(data.name)},</p>
            <p style="color:#444;line-height:1.7;">${escapeHtml(statusText)}</p>
            <p style="color:#666;">Order ID: <strong>${escapeHtml(data.orderId)}</strong></p>
